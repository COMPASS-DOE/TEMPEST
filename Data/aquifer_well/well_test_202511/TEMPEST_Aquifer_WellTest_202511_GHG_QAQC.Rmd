...
#Read in June Data 
raw <- read.csv("Raw Data/TEMPEST_Well_Test_GHG_202511.csv")
head(raw)
...
#CO2 Curve
CO2_stds <- ggplot(stds_co2, aes(Conc_CO2, CO2_Area)) + geom_point(size=4) + 
 geom_smooth(method = "lm", se = FALSE, color="red") + labs(title="CO2 Std Curve")
CO2_stds

stds_co2_lm <- lm(stds_co2$CO2_Area ~ stds_co2$Conc_CO2)
summary(stds_co2_lm)
cf <- coef(stds_co2_lm)
...
Slopes$Run_Date <- c(202511, 202511, 202511)
...
ch4_chk <-  ggplot(data = chks, aes(x = Sample.ID, y = CH4_Conc_ppm, fill=CH4_diff_flag)) +
...
ch4_samples <-  ggplot(data = Samples, aes(x = Sample.ID, y = CH4_Conc_ppm, fill=CH4_Flag)) +
       geom_bar(stat = 'identity') + 
...
#quick first look at the samples 
ch4_samples1 <-  ggplot(data = Samples, aes(x = Sample.ID, y = CH4_Conc_nmol, fill=G_W)) +
       geom_bar(stat = 'identity') + 
...
final_data <- Samples1 %>% 
  ...
  mutate(
    ...
    Replicate = c("A", "B", "A", "B")#,
    # = run_notes     # new column with notes about the run
  ) 
...
ch4_shakeys <-  ggplot(data = final_data, aes(x = sample_name, y = CH4_ppm)) +
       geom_bar(stat = 'identity') + 
        #scale_fill_manual(values=c("darkgrey","darkblue"))+
        #scale_fill_gradient2(low='red', mid='white', high='blue', space='Lab') + 
        theme_classic() + labs(x= " ", y="CH4 (ppm)", title="TMP Aquifer Well") + 
        theme(legend.position="none") 
ch4_shakeys

write.csv(final_data, "Processed Data/TMP_202511_FW_WellTest_GHG_Processed.csv")
