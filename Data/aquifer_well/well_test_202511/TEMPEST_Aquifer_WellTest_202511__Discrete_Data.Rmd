---
title: "TEMPEST_FW_Well_TSS_20251110"
...
#Read in the data 
ghg <- read.csv("Processed Data/TMP_202511_FW_WellTest_GHG_Processed.csv")
#head(doc)

#Plot DOC data 
viz_ch4_plot <-  ggplot(data = ghg, aes(x = Replicate, y = CH4_ppm, fill=Sample_Time)) +
       geom_bar(stat = 'identity') + 
        facet_grid(~Sample_Time, scales="free_x") +
        scale_fill_manual(values = c("#619CFF", "darkblue")) + 
...
#Read in the data 
dic <- read.csv("Processed Data/TMP_20251110_FW_WellTest_DIC_Processed.csv")
#head(doc)
...
#Read in the data 
doc <- read.csv("Processed Data/TMP_20251110_FW_WellTest_DOC_Processed.csv")
#head(doc)
...
#read out title
cat("FE Data from Shimadzu TOC-V")

#Read in the data 
fe <- read.csv("Processed Data/TEMPEST_AquiferWell_FE_data_updated.csv")
#head(fe)

#clean up this data 
fe <- fe %>% 
    filter(str_detect(IDs, "11-10-25")) %>%
    filter(Dilution == 16) %>%
     mutate(
    Project = "COMPASS",   # new column with same value on every row
    Experiment = "TEMPEST: Well Test",
    Sample_Date = "2025-11-10",
    Sample_Time = c("13:30", "15:00"),
...
#Plot DOC data 
viz_fe2_plot <-  ggplot(data = fe, aes(x = Sample_Time, y = FE_2_mean, fill=Sample_Time)) +
       geom_bar(stat = 'identity') + 
        #facet_grid(~Sample_Time, scales="free_x") + 
        scale_fill_manual(values = c("#619CFF", "darkblue")) +
        theme_classic() + labs(x= " ", y="Fe(II) (uM)", title="Well: Fe(II)") + 
        theme(legend.position="right") 
viz_fe3_plot <-  ggplot(data = fe, aes(x = Sample_Time, y = FE_3_mean, fill=Sample_Time)) +
       geom_bar(stat = 'identity') + 
        #facet_grid(~Sample_Time, scales="free_x") + 
        scale_fill_manual(values = c("#619CFF", "darkblue")) +
        theme_classic() + labs(x= " ", y="Fe(III) (uM)", title="Well: Fe(III) - NOT DETECTED") + 
        theme(legend.position="right") 
print(viz_fe2_plot)
print(viz_fe3_plot)
...
cat("Pictures of FE precipitation")

cat("Four TMP Aquifer Well Vials compared to those with surface water from Rhode River:")
knitr::include_graphics("Processed Data/IMG_9725.jpg")

cat("Refridgerated samples from TMP Aquifer Well:")
knitr::include_graphics("Processed Data/IMG_9727.jpg")
knitr::include_graphics("Processed Data/IMG_9728.jpg")
```


\newpage

#STILL WORKING ON THE REST OF THE SAMPLES:
