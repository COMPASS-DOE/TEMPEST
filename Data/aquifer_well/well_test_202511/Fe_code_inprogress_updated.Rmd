...
dat_aver$ID_full <- paste(dat_aver$Plate, dat_aver$Dilution, dat_aver$IDs, sep = "_")
...
#make dataframe for fixed data
  ...
  dat_HCV_fixed_FE2<-data.frame(matrix(ncol = 18, nrow =0 ))
  colnames(dat_HCV_fixed_FE2)<-colnames(dat1)
  dat_HCV_fixed_FE3<-data.frame(matrix(ncol = 18, nrow =0 ))
  ...
  for(x in 1:nrow(dat1_HCV_1)){
    dat_HCV <- subset(dat1, ID_full %in% dat1_HCV_1$ID_full )
    ...
    # delete the point that gives the lowest cv
    sing_ID<-if(which.min(cv_trial)<cv(sing_ID$FE_2_Conc_Final)){sing_ID[-which.min(cv_trial),]}
  ...
  dat1_HCV_fixed_FE2 <- dat_HCV_fixed_FE2 %>%
    group_by(ID_full) %>%
    summarise(IDs=first(IDs),FE_2_mean = mean(FE_2_Conc_Final), FE_2_sd = sd(FE_2_Conc_Final), FE_2_cv = cv(FE_2_Conc_Final),
              FE_3_mean = mean(FE_3_Conc_Final), FE_3_sd = sd(FE_3_Conc_Final), FE_3_cv = cv(FE_3_Conc_Final),
  ...
  dat1_HCV_fixed<-subset(dat_aver1,ID_full %in% dat_HCV_fixed_FE2$ID_full )
  dat1_HCV_fixed<-dat1_HCV_fixed[order(dat1_HCV_fixed$ID_full),]
  dat1_HCV_fixed_FE2 <- dat1_HCV_fixed_FE2[order(dat1_HCV_fixed_FE2$ID_full),]
  
  dat1_HCV_fixed$FE_2_mean <- dat1_HCV_fixed_FE2$FE_2_mean
  dat1_HCV_fixed$FE_2_sd <- dat1_HCV_fixed_FE2$FE_2_sd
  dat1_HCV_fixed$FE_2_cv <- dat1_HCV_fixed_FE2$FE_2_cv
  #Flag high cvs 
  dat1_HCV_fixed$FE_2_cv_flag <- ifelse(dat1_HCV_fixed_FE2$FE_2_cv > 10, "High cv rerun", "ok")
  
  #recombine data
  dat_aver1 <- subset(dat_aver1, !(ID_full %in% dat1_HCV_fixed$ID_full ))
  dat_aver1 <- rbind(dat1_HCV_fixed, dat_aver1)
  head(dat_aver1)
  ...
  for(x in 1:nrow(dat1_HCV_1)){
    ...
    # delete the point that gives the lowest cv
    sing_ID<-if(which.min(cv_trial)<cv(sing_ID$FE_3_Conc_Final)){sing_ID[-which.min(cv_trial),]}
  ...
  #summarize by sampleID so that we can calculate the mean and std. dev. of the three wells 
  dat1_HCV_fixed<-subset(dat_aver1,ID_full %in% dat_HCV_fixed_FE3$ID_full )
  dat1_HCV_fixed<-dat1_HCV_fixed[order(dat1_HCV_fixed$ID_full),]
  dat1_HCV_fixed_FE3 <- dat1_HCV_fixed_FE3[order(dat1_HCV_fixed_FE3$ID_full),]
  
  dat1_HCV_fixed$FE_3_mean <- dat1_HCV_fixed_FE3$FE_3_mean
  dat1_HCV_fixed$FE_3_sd <- dat1_HCV_fixed_FE3$FE_3_sd
  dat1_HCV_fixed$FE_3_cv <- dat1_HCV_fixed_FE3$FE_3_cv
  #Flag high cvs 
  dat1_HCV_fixed$FE_3_cv_flag <- ifelse(dat1_HCV_fixed_FE3$FE_3_cv > 10, "High cv rerun", "ok")
...
write.csv(dat_aver1, "S:/GlobalChangeEco/Porewater_R scripts & Data/Data/Fe/COMPASS/TEMPEST Aquifer Well/QAQC'd Data/TEMPEST_AquiferWell_data_updated.csv")
...
FE3 <- ggplot(dat_aver1, aes(x=ID_full, y=FE_3_mean, color=FE_3_flag))+ 
  geom_point(size=4)+ 
  scale_color_manual(values = c("High" = "red", "Below MDL" = "orange", "Within range" = "black")) +  theme_classic() + 
  labs(y="Fe(II) (uM)", x="Sample ID",title=paste(Project, Experiment," Iron(III) Data")) + 
   geom_errorbar(aes(ymin=FE_3_mean-FE_3_sd,
                    ymax=FE_3_mean+FE_3_sd),width=0.3,position=position_dodge(.1))+
  guides(x = guide_axis(angle = 70))
FE3
