---
title: "COMPASS_TEMPEST_SGW_2025: Well Test #3"
...
#Read in June Data 
raw <- read.csv("Raw Data/TEMPEST_SGAS_Test_Well_20251002.csv")
head(raw)
...
Slopes$Run_Date <- c(202510, 202510) #, 202509)
...
final_data$Minutes <- as.numeric(final_data$Minutes)

#plot the data thru time so we can see the equilibration 
final <-  ggplot(data = final_data, aes(x = Minutes, y = CH4_Conc_ppm_dilcorr, color=Event_Stamp)) +
        geom_point(size=4) + 
        geom_line() + 
        scale_x_continuous(breaks = seq(0, 180, by = 15))+
        theme_classic() + labs(x= "Minutes", y="CH4 (ppm)", title="Gas Well Test #2: 50ppm CH4") + 
