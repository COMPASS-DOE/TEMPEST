---
title: "COMPASS_TEMPEST_TGW_202406: TMP-III D4PM & D5AM"
...
#Read in  Data 
raw <- read.csv("2024 Raw Data/TEMPEST_TGW_Event_D4PM_D5AM_202406_entered.csv")
head(raw)
...
CH4_Low <- ggplot(stds_ch4_low, aes(STD_Conc, CH4_Area)) + geom_point(size=4) + 
  geom_smooth(method = "lm", se = FALSE) + stat_poly_eq() + labs(title="CH4 LOW Std Curve")
CH4_Low
...
CH4_High <- ggplot(stds_ch4, aes(STD_Conc, CH4_Area)) + geom_point(size=4) + 
  geom_smooth(method = "lm", se = FALSE) + stat_poly_eq() + labs(title="CH4 HIGH Std Curve")
CH4_High
...
CO2_stds <- ggplot(stds_co2, aes(STD_Conc, CO2_Area)) + geom_point(size=4) + 
  geom_smooth(method = "lm", se = FALSE, color="red") + stat_poly_eq() +  labs(title="CO2 Std Curve")
CO2_stds
...
Slopes$Run_Date <- c(20241001.5, 20241001.5, 20241001.5)

log <- read.csv("2024 Raw Data/COMPASS_TEMPEST_Varian_TGAS_QAQC_2024.csv")
head(log)
log <- log[ ,-c(1)]
...
write.csv(log, "2024 Raw Data/COMPASS_TEMPEST_Varian_TGAS_QAQC_2024.csv")
```
...
chks$CO2_diff_flag <-  ifelse(chks$Ch4_diff >-10 & chks$Ch4_diff <10 , 'YES', 'NO, rerun')
...
## Dilution correct samples - note same sample IDs mean that they plot together in some cases 
```{r}
...
Samples1 <- Samples[ ,c(1:3,6:18,22:23, 27:34)]
#head(Samples1)

#write out the data to a csv in the processed data folder
# write.csv(Samples1, "2024 Processed Data/COMPASS_TEMPEST_TGW_202406-D4PM-D5AM_Processed.csv")
