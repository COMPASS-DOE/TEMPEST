---
title: "COMPASS_TEMPEST_TGW_2022_CollateData"
author: "Stephanie J. Wilson"
date: "2025-08-29"
output: pdf_document
---

##This code formats the 2022 data for collationand outputs TMP as well as GCW TGAS data. 
...
```{r}
#read in all data: 
dat <- read.csv("2022 Processed Data/TEMPEST_TGW_2022_AllData.csv")
head(dat)
...
final <- dat1 %>%
  select( "Project", "Year","Month" , "Day", "Time","Time_Zone", "Event_Stamp", "Plot", "Sapflux_Tree_ID", 
          ...
          "CH4_ppm", "CH4_Flag", "CO2_ppm", "CO2_Flag", "Lab_Notes")
...
write.csv(final, "COMPASS_TEMPEST_TGW_2022_AllData.csv")
...
syn_trees <- c("C7_H", "C9", "C10", "C11_H", "C12", "C19")

syn_dat <- final %>%
 filter(Event_Stamp %in% "Pre") %>%
 filter(Sapflux_Tree_ID %in% syn_trees)

write.csv(syn_dat, "2022 Processed Data/COMPASS_SynopticCB_GCW_UP_TGAS_2022.csv")
