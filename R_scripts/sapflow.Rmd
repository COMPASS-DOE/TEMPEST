...
if(TESTING) {
  ...
  sf_raw %>% 
    ...
    left_join(inventory, by = c("Tree_Code" = "Sapflux")) %>% 
    select(Timestamp, Record, BattV_Avg, Port, Value, Logger, Tree_Code, Grid, Species) -> sapflow
  
  nomatch_ports <- anti_join(sf_raw, ports, by = c("Logger", "Port"))
 ...
 sf_raw %>% 
    left_join(ports, by = c("Logger", "Port")) %>% 
    anti_join(inventory, by = c("Tree_Code" = "Sapflux")) ->  nomatch_inv
  if(nrow(nomatch_inv) > 0) {
    warning("There were logger/port combinations that I couldn't find in inventory.csv:")
    nomatch_inv %>% 
      distinct(Tree_Code) %>% 
      print()
  }
...
```{r daily-control}
p_daily <- ggplot(filter(sapflow, Plot == "C"), 
...
```{r daily-fresh}
print(p_daily %+% filter(sapflow, Plot == "F"))
```

### Salt

```{r daily-salt}
print(p_daily %+% filter(sapflow, Plot == "S"))
```

### Shore

```{r daily-L}
print(p_daily %+% filter(sapflow, Plot == "L"))
```
...
DAILY_TESTS %>% 
  ...
  left_join(inventory, by = c("Tree_Code" = "Sapflux")) %>% 
  rename("Plot" = "Plot.x") %>% 
  select(test, Plot, Tree_Code, Port, Grid, failures) %>% 
