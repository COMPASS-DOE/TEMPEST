...
# Helper function
range_test <- function(values) {
  !between(values, min(VOLTAGE_RANGE), max(VOLTAGE_RANGE))
}

# Compute daily check if sensor went outside of the expected range
sapflow %>% 
  group_by(Date, Tree_Code, Port) %>% 
  mutate(E1 = range_test(Value)) -> daily_range_test

# Count flags and store results
daily_range_test %>% 
  group_by(Tree_Code, Port) %>%
  summarise(failures = sum(E1, na.rm = TRUE), .groups = "drop") ->
  DAILY_TESTS[["ERR.range"]] # Store results in list

daily_range_test %>% 
  ggplot(aes(x = Timestamp, y = Value, color = E1)) + 
  geom_point() + 
...
sapflow %>% 
  group_by(Date, Tree_Code, Port) %>% 
  mutate(E2 = nan_test(Value)) -> daily_nan_test

# Count flags and store results
daily_nan_test %>% 
  group_by(Tree_Code, Port) %>%
  summarise(failures = sum(E2, na.rm = TRUE), .groups = "drop") ->
  DAILY_TESTS[["ERR.NaN"]] # Store results in list

daily_nan_test %>% 
  ggplot(aes(x = Timestamp, y = Value, color = E2)) + 
  geom_point() + 
  facet_wrap(~Tree_Code, scales = "free")
