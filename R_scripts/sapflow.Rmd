...
if(TESTING) {
  ...
  sf_raw %>% 
    ...
    select(Timestamp, Record, BattV_Avg, Port, Value, Logger, Tree_Code, Grid_Square, Species, Installation_Date) -> sapflow
...
DAILY_TESTS %>% 
  ...
  rename("Plot" = "Plot.x") %>% 
  select(test, Plot, Tree_Code, Port, Grid_Square, Logger, Installation_Date, failures) %>% 
