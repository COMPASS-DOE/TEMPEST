...
inv <- read_csv("../Data/tree_inventory/inventory.csv",
                col_types = cols(
                    ...
                    Date_2026 = col_date(format = ""),
                    DBH_2026 = col_double(),
                    Status_2026 = col_character(),
                    In_Plot = col_logical(),
                    Notes = col_character(),
...
genera <- read_csv("../Data/tree_inventory/species_genera.csv", 
                   col_types = "cc")
...
pivot_and_split <- function(df, col_prefix) {
    ...
    df %>%
        select(Plot, Section, Tree_ID, Grid, Species_code, starts_with(col_prefix)) %>%
...
joincols <- c("Plot", "Section", "Tree_ID", "Grid", "Species_code", "Year")
inv %>%
...
```{r relative-growth, fig.height=8, fig.width=8}
growth_limits <- c(-1.0, 1.0)
inv_long %>% 
    filter(!is.na(Growth)) %>% 
    ggplot(aes(DBH, Growth, color = Genus)) +
...
inv_long %>% 
    filter(Growth < min(growth_limits) | Growth > max(growth_limits)) %>% 
    select(Plot, Section, Tree_ID, Grid, Species_code, Year, Status, DBH, Growth) %>% 
...
inv_long %>% 
    filter(Resurrect) %>% 
    select(Plot, Section, Tree_ID, Grid, Species_code, Year, Status, DBH) %>% 
    knitr::kable()
```
...
inv_long %>%
    ...
    group_by(Plot, Section, Tree_ID, Grid, Species_code) %>% 
