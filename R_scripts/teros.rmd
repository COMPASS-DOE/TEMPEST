...
teros_data %>% 
    ...
    summarise(NAs = sum(!is.finite(M_Value)),
              T_bounds = teros_bounds(M_Value, TEMP_RANGE),
              VWC_bounds = teros_bounds(M_Value, VWC_RANGE),
              EC_bounds = teros_bounds(M_Value, EC_RANGE)) -> teros_summary
...
data.frame(dates, loggers, values) -> test_df

test_df %>% 
    group_by(loggers) %>% 
    summarise(last_online = tail(test_df$dates[which(is.finite(test_df$values))], n = 1))
