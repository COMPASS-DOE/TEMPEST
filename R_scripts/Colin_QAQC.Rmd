...
scatterpie_dist <- ggplot() + geom_scatterpie(aes(X, Y, r = total/18), data = scatter_inventory, cols = colnames(scatter_inventory)[-(1:4)]) + coord_equal() + facet_wrap(~Plot, ncol=1) + scale_x_continuous(breaks = 1:10, labels=c(LETTERS[1:10])) + xlab(NULL)
