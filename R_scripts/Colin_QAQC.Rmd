...
Diam_Distr
```
```{r scatterpie, fig.width=10,fig.height=20, eval=FALSE}
...
scatterpie_dist <- ggplot() + geom_scatterpie(aes(X, Y), data = scatter_inventory, cols = colnames(scatter_inventory)[-(1:4)]) + coord_equal() + facet_wrap(~Plot, ncol=1) + scale_x_continuous(breaks = 1:10, labels=c(LETTERS[1:10])) + xlab(NULL)
...
## Zombie Trees
```{r}
# Table of trees that were dead (DS, DC) in one year and then alive (LI) in a subsequent year
# Groups trees by their tag (groups data by each different tree) and checks if every LI status is before every dead (DS, DC) status by finding their indices and comparing values
...
## Incorrect DBH
```{r}
#Table of trees that are less than 1 cm in diameter (diameter is too small to be considered an accurate measurement)
